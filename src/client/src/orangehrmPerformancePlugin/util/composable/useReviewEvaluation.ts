...
export interface UnidadCalificacion {
  id: number;
  descripcion: string;
  valor: number;
...
export interface UnidadCalificacionFilter {
  descripcion?: string;
  estado?: boolean;
}
...
export default function useReviewEvaluation(http: APIService) {
  ...
  const getAllUnidadCalificaciones = () => {
    return http.request({
      method: 'GET',
      url: `/api/v2/performance/unidades-calificacion`,
    });
  };
  ...
  const generateUnidadCalificacionRules = (unidades: UnidadCalificacion[]) => {
    return unidades.map((unidad) => [
      greaterThanOrEqual(
        1,
        $t('unidadCalificacion.valor_debe_ser_mayor_a', {
          minValue: 1,
        }),
      ),
      lessThanOrEqual(
        unidad.valor,
        $t('unidadCalificacion.valor_debe_ser_menor_o_igual_a', {
          maxValue: unidad.valor,
        }),
      ),
    ]);
  };
  ...
  const generateUnidadCalificacionModel = (unidades: UnidadCalificacion[]) => {
    return {
  ...
  const generateUnidadCalificacionFormData = (
    unidadesData: UnidadCalificacion[],
    generalComment: string,
    unidades: Array<{unidadId: number}>,
  ) => {
    return {
      unidades: unidades.map(({unidadId}) => {
        const _unidad = unidadesData.find((unidad) => unidad.id === unidadId);
        return {
          unidadId,
          descripcion: _unidad?.descripcion || null,
          valor: _unidad?.valor || null,
          estado: _unidad?.estado || null,
        };
      }),
      generalComment: generalComment,
    };
  ...
  return {
    ...
    getAllUnidadCalificaciones,
    generateUnidadCalificacionRules,
    generateUnidadCalificacionModel,
    generateUnidadCalificacionFormData,
  };
}
